...
router.post('/register', function (req, res) {
  ...
  // 处理：判断用户是否已经存在。如果存在，返回错误提示，如果不存在，保存
  //    查询（根据username）
  UserModel.findOne({
    username
  }, function (err, user) {
    ...
    if (user) {
      ...
      new UserModel({
        ...
        // 胜场一个cookie(userid:user._id)，并交给浏览器保存
...
router.post('/login', function (req, res) {
  ...
  // 根据username和password查询数据库users，如果没有，返回提示错误的信息，如果有，返回登陆成功信息（user）
  UserModel.findOne({
      username,
      password: md5(password)
    },
    ...
    function (err, user) {
      if (user) { // 登陆成功
        // 1. 生成一个cookie(userid:user_id)，并交给浏览器保存
        res.cookie('userid', user._id, {
